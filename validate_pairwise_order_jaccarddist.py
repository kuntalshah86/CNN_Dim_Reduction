...
for k in sorted(label_dict.values()):
	...
	f1=open("pairwise_jaccarddist_sorted_orig_dim_%s.dat" % label_dict_reverse[k],"wb");
	f2=open("pairwise_jaccarddist_sorted_PCA256_%s.dat" % label_dict_reverse[k],"wb");
	f3=open("pairwise_jaccarddist_sorted_PCA512_%s.dat" % label_dict_reverse[k],"wb");
	f4=open("pairwise_jaccarddist_sorted_PCA1024_%s.dat" % label_dict_reverse[k],"wb");
	f5=open("pairwise_jaccarddist_sorted_PCA2048_%s.dat" % label_dict_reverse[k],"wb");
	f6=open("pairwise_jaccarddist_sorted_PCA4096_%s.dat" % label_dict_reverse[k],"wb");
	...
	for i in range(0,len(indices)):
		for j in range(i+1,len(indices)):
			...
			dist_orig.append(__Jaccard(X[indices[i]],X[indices[j]]));
			dist_PCA256.append(__Jaccard(X_transformed_256[indices[i]],X_transformed_256[indices[j]]));
			dist_PCA512.append(__Jaccard(X_transformed_512[indices[i]],X_transformed_512[indices[j]]));
			dist_PCA1024.append(__Jaccard(X_transformed_1024[indices[i]],X_transformed_1024[indices[j]]));
			dist_PCA2048.append(__Jaccard(X_transformed_2048[indices[i]],X_transformed_2048[indices[j]]));
			dist_PCA4096.append(__Jaccard(X_transformed_4096[indices[i]],X_transformed_4096[indices[j]]));
			count=count+1;
